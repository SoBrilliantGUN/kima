"""词法检索：pg_jieba 中文全文检索 ts_rank top-k（按 kb_id 过滤）。
...
_NOTE_LEXICAL_SQL = text(
    ...
    WHERE nkb.knowledge_base_id = :kb_id
...
async def lexical_search(
    session: AsyncSession, kb_id: uuid.UUID, query: str, top_k: int
) -> list[RetrievedChunk]:
    """文档 + 笔记两路词法检索，合并返回（各自顺序即 rank）。"""
    hits: list[RetrievedChunk] = []
    hits.extend(await _lexical_documents(session, kb_id, query, top_k))
    hits.extend(await _lexical_notes(session, kb_id, query, top_k))
    return hits
...
async def _lexical_documents(
    ...
    rows = (
        await session.execute(
            _DOCUMENT_LEXICAL_SQL, {"query": query, "kb_id": kb_id, "limit": top_k}
        )
    ).mappings().all()
    return [
        RetrievedChunk(
...
async def _lexical_notes(
    ...
    rows = (
        await session.execute(
            _NOTE_LEXICAL_SQL, {"query": query, "kb_id": kb_id, "limit": top_k}
        )
    ).mappings().all()
    return [
        RetrievedChunk(
