...
from app.models.note import DEFAULT_NOTE_TITLE
...
@pytest.fixture
def service(note_repo: FakeNoteRepository, kb_repo: FakeKnowledgeBaseRepository) -> NoteService:
    return NoteService(note_repo, kb_repo)


async def _make_kb(kb_repo: FakeKnowledgeBaseRepository, name: str = "工作") -> KnowledgeBase:
    return await kb_repo.add(KnowledgeBase(name=name))
...
async def test_update_title_and_content(service: NoteService) -> None:
    note = await service.create_blank(NoteCreate())
    updated = await service.update(note.id, NoteUpdate(title="新标题", content_markdown="# 正文"))
